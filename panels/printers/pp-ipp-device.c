...
//         http_t *http = httpConnect2(data->hostname, data->port, NULL, AF_UNSPEC, HTTP_ENCRYPTION_ALWAYS, 1, 100, NULL);
...
{ 
        ...
        //                         user_data);
        // g_main_loop_run(printer_device_backend->loop);
...
Avahi*
cupsGetIPPDevices (gpointer user_data)
{
    int                 no_sys_objs,
                        no_services,
                        it;
    ...
    printer_device_backend = g_new0(Avahi, 4);
    for (int x = 0; x < 4; x++)
    {
      ...
      printer_device_backend[x].loop =  g_main_loop_new(NULL, FALSE);
      // printer_device_backend[x].user_data = user_data;
    }
    printer_device_backend[0].service_type = "_ipps-system._tcp";
    printer_device_backend[1].service_type = "_ipp-system._tcp";
    printer_device_backend[2].service_type = "_ipps._tcp";
    printer_device_backend[3].service_type = "_ipp._tcp";
